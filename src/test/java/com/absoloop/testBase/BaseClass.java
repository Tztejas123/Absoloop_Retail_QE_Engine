...
public class BaseClass {
    ...
    public void setup(
            ...
            @Optional("https://tutorialsninja.com/demo/") String url,
            @Optional("local")   String execution) {

        startTime.set(System.currentTimeMillis());  // ✅ start timer

        log.info("╔══════════════════════════════════════════════════╗");
        log.info("  TEST STARTING | OS: {} | Browser: {} | Thread: {}",
                os, br, Thread.currentThread().getName());
        log.info("╚══════════════════════════════════════════════════╝");

        WebDriver driver = DriverFactory.createDriver(br, execution);
        DriverManager.setDriver(driver);

        log.info("Driver created: {} | Session: {}",
    ...
    public void tearDown(ITestResult result) {
        ...
        if (result.getStatus() == ITestResult.SUCCESS) {
            log.info("✅ TEST PASSED  | {} | Duration: {}ms | Browser: {}",
                    result.getMethod().getMethodName(),
                    duration,
                    Thread.currentThread().getName());
        } else if (result.getStatus() == ITestResult.FAILURE) {
            log.error("❌ TEST FAILED  | {} | Duration: {}ms | Reason: {}",
                    result.getMethod().getMethodName(),
                    duration,
                    result.getThrowable().getMessage());
        } else {
            log.warn("⚠️ TEST SKIPPED | {} | Duration: {}ms",
