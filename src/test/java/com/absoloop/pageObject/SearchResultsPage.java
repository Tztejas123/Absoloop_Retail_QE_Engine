...
public class SearchResultsPage extends BasePage {
	...
	@FindBy(xpath = "//h2[contains(.,'There is no product')]")
	private WebElement msgNoProduct;
	...
	public boolean isNoResultMsgShown() {
		return isDisplayed(msgNoProduct, "No results message");
	}

	/**
	 * Click a product by name and return ProductDetailPage. Replaces old void
	 * clickProductByName().
	 */
	public ProductDetailPage selectProduct(String name) {
		productNames.stream().filter(e -> e.getText().trim().equalsIgnoreCase(name)).findFirst().ifPresent(e -> {
			click(e, "Product: " + name);
